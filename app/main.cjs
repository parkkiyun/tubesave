...
async function setupUpdater(dataDir){
  ...
  try {
    ...
    if(distribution.format==='personal-ed25519'&&app.isPackaged){
      const {PersonalUpdater,configValid}=require('./lib/personal-updater.cjs');
      if(!configValid(distribution))throw Error('Personal update public key missing.');
      adapter=new PersonalUpdater({config:distribution,version:VERSION,dataDir,resourcesPath:process.resourcesPath,executable:process.execPath,quit:()=>app.quit()});
      await adapter.completeReceipt();
      reason='';
    }else if(distribution.format==='electron-builder'&&app.isPackaged&&validReleaseConfig(distribution)){
