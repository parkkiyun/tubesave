...
async function main(){
 ...
 const result=path.join(dir,'result.json'),{privateKey}=crypto.generateKeyPairSync('ed25519');let manifest,archive,child,served=0;
 ...
 try{
  ...
  console.log('NATIVE_UPDATE_STAGE build-new',process.platform,process.arch);
  ...
  console.log('NATIVE_UPDATE_STAGE build-old');
  ...
  console.log('NATIVE_UPDATE_STAGE launch-old');
  child=spawn(exe,[],{stdio:'inherit',shell:false});child.on('error',e=>console.error(e.message));child.unref();
 ...
 }finally{server.closeAllConnections();server.close();if(child&&child.exitCode===null)child.kill('SIGTERM');}
}
